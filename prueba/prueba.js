...
describe('eCommerce', () => {
  ...
  before((done) => {
    this.productos = new ContenedorArchivo('Productos.json')
    this.carritos = new ContenedorArchivo('Carritos.json')
  ...
  describe('API productos', () => {
    ...
    it('obtener producto por id', async () => {
      const producto = { p: 1 }
      const id = await this.productos.save(producto)
      producto.id = id
    ...
    it('crear producto', async () => {
      ...
      const actuales = await this.productos.getAll()
    ...
    it('actualizar producto', async () => {
      ...
      const guardado = await this.productos.getById(id)
    ...
    it('eliminar producto', async () => {
      ...
      const guardados = await this.productos.getAll()
  ...
  describe('API carritos', () => {
    ...
    it('eliminar carrito', async () => {
      ...
      const id = await this.carritos.save(carrito)
    ...
    it('agregar un producto al carrito', async () => {
      ...
      const idCarrito = await this.carritos.save(carrito)
      carrito.id = idCarrito

      const producto = { p: 1 }
      const idProd = await this.productos.save(producto)
      producto.id = idProd

      await axios.post(`${url}/api/carritos/${idCarrito}/productos`, { id: idProd })

      const carritoGuardado = await this.carritos.getById(idCarrito)
